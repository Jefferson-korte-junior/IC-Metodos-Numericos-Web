// components/newton/NewtonInterativo.jsx
// ─────────────────────────────────────────────────────────────────────────────
// Modo interativo passo-a-passo para Newton-Raphson.
// Os valores de f(x), f'(x), x_novo e erro vêm do backend — nunca são
// recalculados aqui. O componente apenas guia o aluno a reproduzi-los.
//
// Props:
...
//   tolerancia      : number  — ε
//   iteracoesBackend: Array   — [{ iteracao, x, fx, dfx, x_novo, erro }, ...]
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Design tokens (idênticos ao BissecaoInterativa) ──────────────────────────
const T = {
  bg:           "#f1f5f9",
  bgWhite:      "#ffffff",
  bgCard:       "#ffffff",
...
// ── Calculadora científica (auxiliar pedagógico) ──────────────────────────────
...
// subpassos Newton: [0]=fx, [1]=dfx, [2]=x_novo, [3]=erro
function PainelEstadoNewton({ fase, subIdx, respondido }) {
  if (!fase) return null;
  ...
  const colunas = [
    ...
    { header: "xₙ₊₁",  value: sp[2]?.esperado,    revelaEm: 3 },
    { header: "erro",   value: sp[3]?.esperado,    revelaEm: 4 },
  ];
...
function construirPassos(funcao, criterio, tolerancia, iteracoesBackend) {
  return iteracoesBackend.map(it => {
    const { iteracao, x, fx, dfx, x_novo, erro } = it;
    ...
    const erroSubpasso = (() => {
      if (criterio === "relativo") {
        return {
          ...
          contextoSubpasso: "O erro relativo normaliza a diferença pelo valor atual — útil quando a raiz tem grande magnitude.",
        };
      }
      if (criterio === "funcao") {
        return {
          perguntaTexto: "erro = |f(xₙ)|",
          pergunta: `Calcule o erro: |f(xₙ)| = |f(${fmt(x, 6)})|`,
          dica: `|${fmt(fx, 6)}|`,
          contextoSubpasso: "|f(xₙ)| mede diretamente quão perto estamos de um zero da função.",
        };
      }
      return {
      ...
      subpassos: [
        {
          ...
          esperado: fx,
          tipo: "numero",
          contextoSubpasso: "O resíduo f(xₙ) indica quão longe estamos de f(x) = 0.",
        },
        {
          id: `i${iteracao}_dfx`,
          perguntaTexto: `f′(xₙ) = f′(${fmt(x, 6)})`,
          pergunta: `Avalie a derivada f′(xₙ): calcule f′(${fmt(x, 6)})`,
          dica: `Derive f(x) = ${funcao} e substitua x = ${fmt(x, 6)}`,
          esperado: dfx,
          tipo: "numero",
          contextoSubpasso: "A derivada f′(xₙ) é a inclinação da reta tangente em xₙ. Se f′(xₙ) ≈ 0, o método pode divergir.",
        },
        {
          id: `i${iteracao}_xnovo`,
          perguntaTexto: `xₙ₊₁ = ${fmt(x, 6)} − f/f′`,
          pergunta: `Aplique a fórmula: xₙ₊₁ = ${fmt(x, 6)} − (${fmt(fx, 6)}) / (${fmt(dfx, 6)})`,
          dica: `${fmt(x, 6)} − (${fmt(fx, 6)}) ÷ (${fmt(dfx, 6)}) = ${fmt(x_novo, 8)}`,
          esperado: x_novo,
          tipo: "numero",
          contextoSubpasso: "xₙ₊₁ é o ponto onde a reta tangente a f em xₙ cruza o eixo x.",
        },
        {
          id: `i${iteracao}_erro`,
          ...erroSubpasso,
...
export default function NewtonInterativo({ funcao, x0, criterio, tolerancia, iteracoesBackend = [] }) {
    ...
    <div style={{ fontFamily:T.fontSans, background:T.bg, minHeight:"100vh", color:T.text, padding:"24px 20px", boxSizing:"border-box" }}>
      ...
      <div style={{ maxWidth: 1120 }}>
        ...
        <div style={{ background:T.bgCard, border:`1px solid ${T.border}`, borderRadius:T.radius, padding:"14px 20px", marginBottom:18, display:"flex", alignItems:"center", gap:16, flexWrap:"wrap", boxShadow:T.shadow }}>
          <div>
            <div style={{ fontSize:18, fontWeight:700, color:T.text, fontFamily:T.fontSans }}>Método de Newton-Raphson</div>
            <div style={{ fontSize:14, color:T.textMuted, fontFamily:T.font, marginTop:3 }}>
              f(x) = {funcao}&nbsp;&nbsp;·&nbsp;&nbsp;x₀ = {fmt(x0, 6)}&nbsp;&nbsp;·&nbsp;&nbsp;ε = {tol}
            </div>
          </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 240px", gap:18, alignItems:"start" }}>
          ...
          <div>
            ...
            {fase && <PainelEstadoNewton fase={fase} subIdx={subIdx} respondido={respondido} />}
            ...
            {concluido ? (
              <div style={{ background:T.greenLight, border:`1px solid ${T.greenBorder}`, borderRadius:T.radius, padding:"30px 28px", boxShadow:T.shadow }}>
                ...
                <h3 style={{ color:T.green, marginBottom:12, fontSize:22, fontFamily:T.fontSans }}>Newton-Raphson concluído!</h3>
