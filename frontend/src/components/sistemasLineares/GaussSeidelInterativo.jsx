// components/sistemasLineares/GaussSeidelInterativo.jsx
// ─────────────────────────────────────────────────────────────────────────────
// Modo interativo passo-a-passo para o Método de Gauss-Seidel.
// Diferença-chave do Jacobi: ao calcular x_i, os valores x_0…x_{i-1}
// já atualizados NESTA iteração são usados (↑), não os da iteração anterior.
//
// Props:
...
// Coluna "x^(k+1) novo" mostra ↑ para valores já calculados nesta iteração
function PainelEstadoSistema({ fase, subIdx, respondido }) {
  if (!fase) return null;
      ...
      <table style={{ borderCollapse:"collapse", fontSize:13, width:"100%" }}>
        <thead>
          <tr style={{ background:T.bgSection }}>
            ...
            <th style={{ padding:"7px 12px", textAlign:"left", fontWeight:600, color:T.green, fontFamily:T.font, borderBottom:`2px solid ${T.primaryBorder}` }}>x^(k+1) novo ↑=já atualizado</th>
          </tr>
        </thead>
        <tbody>
          {fase.xPrev.map((xp, i) => {
            ...
            return (
              <tr key={i} style={{ background: i % 2 === 0 ? "white" : T.bgSection }}>
                ...
                <td style={{ padding:"8px 12px", fontFamily:T.font, fontWeight: revelado ? 700 : 400, color: revelado ? T.green : T.textFaint }}>
                  {revelado
                    ? <>{fmtS(fase.xNovo[i], 6)} <span style={{ color:T.teal, fontSize:11 }}>↑</span></>
                    : "?"
                  }
                </td>
              </tr>
...
function buildFormula(A, b, xPrev, xJaAtualizado, i) {
  const n = A.length;
  let expr = `(${fmt(b[i], 4)}`;
  for (let j = 0; j < n; j++) {
    if (j === i) continue;
    ...
    if (c === 0) continue;
    const sinal = c > 0 ? " − " : " + ";
    const val = j < i ? xJaAtualizado[j] : xPrev[j];
    const tag = j < i ? "↑" : "";
    expr += `${sinal}(${fmt(Math.abs(c), 4)})×(${fmt(val, 4)}${tag})`;
  }
  expr += `) / ${fmt(A[i][i], 4)}`;
  return expr;
}

function buildDica(A, b, xPrev, xJaAtualizado, i, xNovo) {
  const n = A.length;
  let num = b[i];
  ...
  for (let j = 0; j < n; j++) {
    ...
    const val = j < i ? xJaAtualizado[j] : xPrev[j];
    const tag = j < i ? "↑" : "";
    exprNum += `${sinal}${fmt(Math.abs(c), 4)}×${fmt(val, 4)}${tag}`;
    num -= c * val;
  }
  const linhas = [
    `Numerador: ${exprNum} = ${fmtS(num, 6)}`,
    `Denominador: ${fmt(A[i][i], 4)}`,
    `x${SUB[i]} = ${fmtS(num, 6)} ÷ ${fmt(A[i][i], 4)} = ${fmtS(xNovo, 8)}`,
  ];
  if (i > 0) {
    const atualizados = Array.from({ length: i }, (_, j) => `x${SUB[j]} = ${fmtS(xJaAtualizado[j], 6)} ↑`).join(", ");
    linhas.push(`\nValores já atualizados usados (↑): ${atualizados}`);
  }
  return linhas.join("\n");
}

// ── Constrói o contextoSubpasso para cada variável ────────────────────────────
function buildContextoSubpasso(k, i, xJaAtualizado) {
  if (k === 0 && i === 0) {
    return "Em Gauss-Seidel, ao contrário de Jacobi, os valores calculados DENTRO da mesma iteração são usados imediatamente. Aqui é a primeira variável, portanto ainda não há valores atualizados nesta iteração.";
  }
  if (i === 0) {
    return "Gauss-Seidel: primeira variável desta iteração — ainda sem valores atualizados nesta iteração. Todos os x usados são da iteração anterior.";
  }
  const lista = Array.from({ length: i }, (_, j) => `x${SUB[j]} = ${fmtS(xJaAtualizado[j], 6)}`).join(",  ");
  return `Gauss-Seidel: valores já calculados nesta iteração (↑) e usados agora: ${lista}`;
}
  ...
  return iteracoesBackend.map((it, k) => {
    ...
    const subpassos = it.valores.map((xNovo, i) => {
      ...
      return {
        ...
        pergunta: `Calcule x${SUB[i]}^(${k + 1}) = ${buildFormula(A, b, xPrev, xJaAtualizado, i)}`,
        dica: buildDica(A, b, xPrev, xJaAtualizado, i, xNovo),
        esperado: xNovo,
        tipo: "numero",
        contextoSubpasso: buildContextoSubpasso(k, i, xJaAtualizado),
      };
    });
    ...
    <div style={{ fontFamily:T.fontSans, background:T.bg, minHeight:"100vh", color:T.text, padding:"24px 20px", boxSizing:"border-box" }}>
      ...
      <div style={{ maxWidth: 1100 }}>
        ...
        <div style={{ background:T.bgCard, border:`1px solid ${T.border}`, borderRadius:T.radius, padding:"14px 20px", marginBottom:18, display:"flex", alignItems:"center", gap:16, flexWrap:"wrap", boxShadow:T.shadow }}>
          <div>
            <div style={{ fontSize:18, fontWeight:700, color:T.text, fontFamily:T.fontSans }}>Método de Gauss-Seidel — Interativo</div>
            <div style={{ fontSize:13, color:T.textMuted, fontFamily:T.font, marginTop:3 }}>
              Sistema {n}×{n}&nbsp;&nbsp;·&nbsp;&nbsp;ε = {tolerancia}&nbsp;&nbsp;·&nbsp;&nbsp;{total} iteração(ões)&nbsp;&nbsp;·&nbsp;&nbsp;<span style={{ color:T.teal }}>↑ = valor já atualizado nesta iteração</span>
            </div>
          </div>
          ...
          <div>
            ...
            {concluido ? (
              <div style={{ background:T.greenLight, border:`1px solid ${T.greenBorder}`, borderRadius:T.radius, padding:"28px", boxShadow:T.shadow }}>
                ...
                <h3 style={{ color:T.green, marginBottom:12, fontSize:20, fontFamily:T.fontSans }}>Gauss-Seidel concluído!</h3>
              ...
              <div style={{ background:T.bgCard, border:`1px solid ${T.border}`, borderRadius:T.radius, padding:"20px 24px", boxShadow:T.shadow }}>
                ...
                {subpasso.contextoSubpasso && (
                  <div style={{ background:T.tealLight, border:`1px solid ${T.tealBorder}`, borderLeft:`3px solid ${T.teal}`, borderRadius:T.radiusSm, padding:"10px 14px", marginBottom:16, fontSize:13, color:T.textMuted, lineHeight:1.65, fontFamily:T.fontSans }}>
                    {subpasso.contextoSubpasso}
                  </div>
                )}
