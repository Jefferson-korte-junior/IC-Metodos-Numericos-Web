...
function Calculadora() {
  ...
  function calcular() {
    try {
      const full = (newNum ? expr : expr + display).trim();
      // eslint-disable-next-line no-new-func
      const res = Function(`return (${full || display})`)();
  ...
  function Btn({ label, onClick, theme="num", wide=false }) {
    const t=th[theme]; const [hov,setHov]=useState(false);
    return (
...
function PainelHistorico({ historico }) {
  ...
  return (
    <div style={{marginTop:20}}>
      ...
      <div ref={ref} style={{background:T.bgSection,border:`1px solid ${T.border}`,borderRadius:T.radius,padding:14,maxHeight:240,overflowY:"auto",display:"flex",flexDirection:"column",gap:12,boxShadow:"inset 0 1px 3px rgba(0,0,0,0.04)"}}>
        {grupos.map((g,gi) => (
          <div key={gi}>
...
// ── Painel de estado — tabela de avaliação revelada progressivamente ──────────
function PainelEstadoTrapezio({ pontos, h, resultado, faseIdx, subIdx, respondido }) {
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,fontFamily:T.font}}>
            ...
            <tbody>
              {pontos.map((p,i) => {
                const rev = isFiRev(i);
                return (
                  <tr key={i} style={{background: rev ? (p.peso===1?T.tealLight:"#fafafa") : "transparent"}}>
                    ...
                    <td style={{padding:"5px 8px",textAlign:"right",fontWeight: rev?700:400,color: rev?T.teal:T.textFaint}}>
                      {rev ? fmtS(p.fi,6) : "?"}
                    </td>
                    <td style={{padding:"5px 8px",textAlign:"center",color: p.peso===1?T.teal:T.primary,fontWeight:700}}>{p.peso}</td>
                    <td style={{padding:"5px 8px",textAlign:"right",color: rev?T.text:T.textFaint}}>
                      {rev ? fmtS(p.fi * p.peso, 6) : "?"}
                    </td>
                  </tr>
                );
              })}
...
function construirPassos(dadosBE) {
  ...
  fases.push({
    ...
    contexto: [{
      titulo: "Regra dos Trapézios Composta",
      texto:
        `I ≈ (h/2) × [f(x₀) + 2f(x₁) + 2f(x₂) + ... + 2f(xₙ₋₁) + f(xₙ)]\n\n` +
        `Endpoints recebem peso 1; nós interiores recebem peso 2.\n` +
        `h = (b − a) / n = (${b} − ${a}) / ${n}`,
    }],
    subpassos: [{
      id: "h",
      ...
      dica: `h = (b − a) / n = (${b} − ${a}) / ${n}\n  = ${b - a} / ${n}\n  = ${fmtS(h, 8)}`,
      esperado: h,
      contextoSubpasso: "h é a largura de cada subintervalo — determina a base de cada trapézio.",
    }],
  });

  // Fase 1: f(xi) para cada ponto
  const subpassosAval = pontos.map((p, i) => ({
    ...
    contextoSubpasso:
      i === 0
        ? `x₀ = a = ${fmtS(a, 4)} — ponto inicial, peso 1 na fórmula.`
        : i === n
        ? `xₙ = b = ${fmtS(b, 4)} — ponto final, peso 1 na fórmula.`
        : `x${subStr(i)} é um nó interior com peso 2 — cada subinterval interno conta duas vezes.`,
  }));
  ...
  fases.push({
    ...
    contexto: [{
      titulo: "Nós da partição",
      texto: pontos.map((p, i) => `x${subStr(i)} = ${fmtS(p.xi, 4)}  (peso ${p.peso})`).join("\n"),
    }],
    subpassos: subpassosAval,
  });

  // Fase 2: resultado
  const termos = pontos.map((p, i) =>
    p.peso === 1 ? fmtS(p.fi, 4) : `${p.peso}·${fmtS(p.fi, 4)}`
  );
  fases.push({
    faseLabel: "Resultado",
    ...
    contexto: [{
      ...
      texto:
        ...
        `I ≈ (h/2) × Σ = (${fmtS(h, 4)}/2) × ${fmtS(soma_ponderada, 6)}`,
    }],
    subpassos: [{
      id: "resultado",
      perguntaTexto: "I",
      pergunta: `I ≈ (${fmtS(h, 4)}/2) × ${fmtS(soma_ponderada, 6)} = ?`,
      dica:
        `I = (h/2) × Σpeso·f(xᵢ)\n` +
        `  = (${fmtS(h, 6)}/2) × ${fmtS(soma_ponderada, 6)}\n` +
        `  = ${fmtS(h / 2, 6)} × ${fmtS(soma_ponderada, 6)}\n` +
        `  = ${fmtS(resultado, 8)}`,
      esperado: resultado,
      contextoSubpasso: "O resultado é a soma das áreas dos trapézios formados pelos segmentos entre pontos consecutivos.",
    }],
  });
...
export default function TrapezioInterativo({ dadosBE }) {
    ...
    <div style={{fontFamily:T.fontSans,background:T.bg,color:T.text,padding:"24px 20px",boxSizing:"border-box"}}>
      ...
      <div style={{maxWidth:1100}}>
        ...
        <div style={{background:T.bgCard,border:`1px solid ${T.border}`,borderRadius:T.radius,padding:"14px 20px",marginBottom:18,display:"flex",alignItems:"center",gap:16,flexWrap:"wrap",boxShadow:T.shadow}}>
          <div>
            <div style={{fontSize:18,fontWeight:700,color:T.text,fontFamily:T.fontSans}}>Regra dos Trapézios — Modo Interativo</div>
            ...
            {concluido ? (
              <div style={{background:T.greenLight,border:`1px solid ${T.greenBorder}`,borderRadius:T.radius,padding:"28px",boxShadow:T.shadow}}>
                ...
                <p style={{fontSize:15,color:T.textMuted,lineHeight:1.7,fontFamily:T.fontSans,marginBottom:16}}>
                  A Regra dos Trapézios com <strong style={{color:T.text}}>{dadosBE.n}</strong> subintervalos resultou em:
                </p>
                ...
                <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
                  ...
                  <div style={{background:"white",border:`1px solid ${T.border}`,borderRadius:T.radiusSm,padding:"10px 16px"}}>
                    ...
                    <div style={{fontSize:16,fontWeight:700,color:T.primary,fontFamily:T.font}}>{fmtS(dadosBE.soma_ponderada, 6)}</div>
                  </div>
                  <div style={{background:"white",border:`1px solid ${T.border}`,borderRadius:T.radiusSm,padding:"10px 16px"}}>
                    <div style={{fontSize:11,color:T.textFaint,fontFamily:T.fontSans,marginBottom:3}}>n subintervalos</div>
              ...
              <div style={{background:T.bgCard,border:`1px solid ${T.border}`,borderRadius:T.radius,padding:"20px 24px",boxShadow:T.shadow}}>
                ...
                <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:16}}>
                  ...
                  />
                  {!respondido
                    ? <button onClick={()=>confirmar()} style={{padding:"10px 22px",background:T.primary,color:"white",border:"none",borderRadius:T.radiusSm,fontSize:15,fontFamily:T.fontSans,fontWeight:600,cursor:"pointer"}}>Verificar</button>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div style={{background:T.bgCard,border:`1px solid ${T.border}`,borderRadius:T.radius,padding:"12px 14px",boxShadow:T.shadow}}>
              ...
              <div style={{fontSize:12,fontFamily:T.font,color:T.textMuted,lineHeight:2}}>
                ...
                <div style={{paddingLeft:14}}>2f(x₁) + ... +</div>
                <div style={{paddingLeft:14}}>2f(xₙ₋₁) +</div>
                <div style={{paddingLeft:14}}>f(xₙ)]</div>
              </div>
              <div style={{marginTop:10,padding:"8px 10px",background:T.tealLight,borderRadius:T.radiusXs,fontSize:12,fontFamily:T.font,color:T.teal}}>
