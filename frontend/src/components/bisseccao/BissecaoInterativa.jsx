// components/bisseccao/BissecaoInterativa.jsx
// ─────────────────────────────────────────────────────────────────────────────
// RESPONSABILIDADE DESTE ARQUIVO
//   • Exibir o modo interativo passo-a-passo da bisseção
//   • Construir os passos didáticos a partir dos dados do backend
//   • NÃO avaliar f(x), NÃO rodar iterações matemáticas localmente
//
// CONTRATO DE PROPS
//   funcao          : string  — expressão original (exibição/dicas)
//   a               : number  — extremo esquerdo original
//   b               : number  — extremo direito original
//   tolerancia      : number  — ε usado no cálculo
//   iteracoesBackend: Array   — histórico completo vindo do backend
//     [{ iteracao, a, b, media, fa, fb, fm, substituiu }, ...]
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── Gráfico SVG ─────────────────────────────────────────────────────────────
// Renderiza o intervalo atual. Os pontos (a, m, b) e os valores f(a), f(m),
// f(b) vêm SEMPRE do backend — nunca são recalculados aqui.
function GraficoBissecao({ iteracaoAtual, aOriginal, bOriginal }) {
  const W = 640, H = 230;
  const PAD = { top: 18, right: 22, bottom: 38, left: 52 };
  const plotW = W - PAD.left - PAD.right;
  const plotH = H - PAD.top - PAD.bottom;

  if (!iteracaoAtual) return null;

  const { a, b, media: m, fa, fb, fm } = iteracaoAtual;

  // ── Escala Y baseada nos valores reais vindos do backend ──────────────────
  const allY = [fa, fb, fm].filter(isFinite);
  if (!allY.length) return null;

  const yMin = Math.min(...allY);
  const yMax = Math.max(...allY);
  const yPad = Math.max(Math.abs(yMax - yMin) * 0.3, 0.5);
  const yLo  = yMin - yPad;
  const yHi  = yMax + yPad;

  const xRange = bOriginal - aOriginal || 1;

  const toSvg = (xv, yv) => ({
    sx: PAD.left + ((xv - aOriginal) / xRange) * plotW,
    sy: PAD.top  + (1 - (yv - yLo) / (yHi - yLo)) * plotH,
  });
  ...
  const faY = toSvg(a, fa).sy;
  const fbY = toSvg(b, fb).sy;
  const fmY = toSvg(m, fm).sy;

  const pts = [
    { sx: aX, sy: faY, color: "#2563eb", label: "a" },
    { sx: mX, sy: fmY, color: "#d97706", label: "m" },
    { sx: bX, sy: fbY, color: "#dc2626", label: "b" },
  ];

  const tickCount = 5;
  const xTicks = Array.from({ length: tickCount + 1 }, (_, i) => {
    const xv = aOriginal + (i / tickCount) * xRange;
    return { xv, sx: toSvg(xv, 0).sx };
  });
      ...
      {xTicks.map(({ xv, sx }) => (
        <g key={xv}>
          ...
          <text x={sx} y={PAD.top + plotH + 16} textAnchor="middle"
            fontSize="10" fill="#64748b" fontFamily={T.font}>{fmt(xv, 3)}</text>
        </g>
      ))}
...
// A calculadora é um auxiliar pedagógico para o aluno verificar as contas à mão.
// Ela NÃO alimenta nenhum estado do componente principal.
    ...
    return (
      <button onClick={onClick}
        ...
        style={{
          ...
          fontSize: 13, fontFamily: theme === "num" || theme === "op" || theme === "eq" ? T.font : T.fontSans,
...
// MUDANÇA CRÍTICA: antes, esta função calculava tudo localmente (avaliar, Math.log10...).
// Agora, recebe `iteracoesBackend` e apenas MONTA a estrutura de perguntas/respostas
// a partir dos dados já calculados pelo backend.
function construirPassos(funcao, a, b, tol, iteracoesBackend) {
  a = +a; b = +b; tol = +tol;
  const passos = [];

  // ── Fase 0: estimativa do número de iterações ─────────────────────────────
  // Os valores intermediários de log são calculados aqui pois são
  // metadados pedagógicos (quanto iterar), não f(x). Não há risco de
  // inconsistência porque não dependem da função do usuário.
  ...
  passos.push({
    ...
    contexto: [
      ...
      },
      {
        ...
        texto:  `Vamos calcular n = ${nMin} como garantia. A raiz pode ser encontrada antes — se f(m) = 0 exatamente, ou se o erro já ficou abaixo de ε.`,
      },
    ],
    subpassos: [
      {
      ...
      },
      {
        ...
        dica:          `Use o botão "log" da calculadora com o valor ${fmt(b-a,5)}`,
        esperado:      logBA,
        tipo:          "numero",
      ...
      },
      {
        ...
        dica:          `Use o botão "log" da calculadora com o valor ${tol}`,
        esperado:      logTol,
        tipo:          "numero",
      ...
      },
      {
        ...
        inteiro:       true,
        tipo:          "numero",
        contextoSubpasso: `Esse valor garante erro < ε = ${tol}.`,
      },
    ],
  });

  // ── Fases 1..N: uma por iteração, usando dados do backend ─────────────────
  for (const it of iteracoesBackend) {
    const { iteracao, a: ai, b: bi, media: med, fa, fb, fm, substituiu } = it;
    const mesmoSinal = substituiu === "a"; // backend já resolveu isso
    const erroAbs    = Math.abs(bi - ai) / 2;
    const convergiu  = Math.abs(fm) <= tol || erroAbs < tol;

    passos.push({
      faseLabel:    `Iteração ${iteracao + 1}`,
      subtitulo:    `Iteração ${iteracao + 1}`,
      ehEstimativa: false,
      estadoA:      ai,
      estadoB:      bi,
      mediaFinal:   med,
      erroAbs,
      convergiu,
      // Passamos a iteração inteira para o gráfico
      iteracaoDados: it,
      contexto: [
        {
          titulo: "Intervalo atual",
          texto:  `a = ${fmt(ai,6)},   b = ${fmt(bi,6)}\nComprimento = ${fmt(bi-ai,6)}.   Erro máximo possível = ${fmt(erroAbs,6)}.`,
        },
      ],
      subpassos: [
        {
          ...
          // ← valor vem do backend, não de eval() local
          esperado:      fa,
          tipo:          "numero",
          contextoSubpasso: iteracao === 0
        ...
        },
        {
          ...
          esperado:      med,
          tipo:          "numero",
          contextoSubpasso: "O ponto médio m é a estimativa atual da raiz.",
        },
        {
          id:            `i${iteracao}_fm`,
        ...
        },
        {
          ...
          mesmoSinal,
          tipo:          "simnao",
          contextoSubpasso: "A comparação de sinais determina em qual metade do intervalo a raiz se encontra.",
        },
        {
          id:            `i${iteracao}_sub`,
      ...
      {aberto && (
        <div style={{ marginTop:8, background:T.yellowLight, border:`1px solid ${T.yellowBorder}`, borderRadius:T.radiusSm, padding:"10px 14px" }}>
          ...
          {completa ? (
            ...
            <>
              <p style={{ margin:0, fontSize:14, color:T.textMuted, lineHeight:1.7, fontFamily:T.fontSans }}>{dica.split("→")[0].split(":")[0].trim()}</p>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:18 }}>
      {itens.map((item, i) => (
        <div key={i} style={{ background:T.primaryLight, border:`1px solid ${T.primaryBorder}`, borderLeft:`3px solid ${T.primary}`, borderRadius:T.radiusSm, padding:"11px 15px" }}>
          ...
          <p style={{ margin:0, fontSize:14, color:T.textMuted, lineHeight:1.65, fontFamily:T.fontSans }}>{item.texto}</p>
        </div>
      ))}
    </div>
  );
...
function PainelHistorico({ historico }) {
  ...
  historico.forEach(item => {
    ...
    else grupos.push({ faseLabel:item.faseLabel, itens:[item], estadoA:item.estadoA, estadoB:item.estadoB });
  });
  ...
  return (
    <div style={{ marginTop:20 }}>
      ...
      <div ref={ref} style={{ background:T.bgSection, border:`1px solid ${T.border}`, borderRadius:T.radius, padding:14, maxHeight:320, overflowY:"auto", display:"flex", flexDirection:"column", gap:16, boxShadow:"inset 0 1px 3px rgba(0,0,0,0.04)" }}>
        {grupos.map((grupo, gi) => (
          <div key={gi}>
            <div style={{ fontSize:12, fontWeight:700, color:T.primary, textTransform:"uppercase", letterSpacing:0.5, marginBottom:8, paddingBottom:6, borderBottom:`1px solid ${T.primaryBorder}`, fontFamily:T.fontSans, display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
              <span>{grupo.faseLabel}</span>
              {grupo.estadoA !== undefined && grupo.estadoB !== undefined && (
                <span style={{ fontWeight:500, color:T.textMuted, fontSize:12, fontFamily:T.font }}>
...
export default function BissecaoInterativa({ funcao, a, b, tolerancia, iteracoesBackend = [] }) {
  const an  = +a;
  const bn  = +b;
  const tol = +tolerancia;

  // Passos construídos UMA VEZ a partir dos dados do backend
  const passos = useRef(null);
  if (!passos.current) {
    passos.current = construirPassos(funcao, an, bn, tol, iteracoesBackend);
  }
  ...
  // Gráfico: usa dados da iteração atual vindos do backend
  const iteracaoGrafico = fase?.ehEstimativa
    ? null
    : fase?.iteracaoDados ?? null;
  ...
  return (
    <div style={{ fontFamily:T.fontSans, background:T.bg, minHeight:"100vh", color:T.text, padding:"24px 20px", boxSizing:"border-box", alignSelf:"flex-start", marginLeft:0, marginRight:"auto" }}>
      <style>{`
      ...
      <div style={{ maxWidth: 1120 }}>
        ...
        <div style={{ background:T.bgCard, border:`1px solid ${T.border}`, borderRadius:T.radius, padding:"14px 20px", marginBottom:18, display:"flex", alignItems:"center", gap:16, flexWrap:"wrap", boxShadow:T.shadow }}>
          <div>
            <div style={{ fontSize:18, fontWeight:700, color:T.text, fontFamily:T.fontSans }}>Método da Bisseção</div>
          ...
          <div>
            ...
            <div style={{ background:T.bgCard, border:`1px solid ${T.border}`, borderRadius:T.radius, padding:"14px 16px", marginBottom:16, boxShadow:T.shadow }}>
              ...
              {iteracaoGrafico ? (
                ...
                <div style={{ height:80, display:"flex", alignItems:"center", justifyContent:"center", color:T.textFaint, fontSize:14, fontFamily:T.fontSans }}>
                  Inicie as iterações para ver o gráfico
                </div>
              )}
              <div style={{ display:"flex", gap:20, marginTop:10, flexWrap:"wrap" }}>
                {[
                  ...
                  <div key={label} style={{ display:"flex", alignItems:"center", gap:6, fontSize:13, color:T.textMuted, fontFamily:T.fontSans }}>
                    <div style={{ width:11, height:11, borderRadius:"50%", background:cor, border:"2px solid white", boxShadow:`0 0 0 1.5px ${cor}` }} />
                    {label}
                  </div>
                ))}
              </div>
            ...
            {concluido ? (
              <div style={{ background:T.greenLight, border:`1px solid ${T.greenBorder}`, borderRadius:T.radius, padding:"30px 28px", boxShadow:T.shadow }}>
                ...
                <h3 style={{ color:T.green, marginBottom:12, fontSize:22, fontFamily:T.fontSans }}>Bisseção concluída!</h3>
                <p style={{ fontSize:16, color:T.textMuted, lineHeight:1.7, fontFamily:T.fontSans }}>
                  Após <strong style={{ color:T.text }}>{total - 1}</strong> iteração(ões), a raiz aproximada é{" "}
                ...
                {feedback?.ok && (
                  <div style={{ background:T.greenLight, border:`1px solid ${T.greenBorder}`, borderRadius:T.radiusSm, padding:"10px 14px", marginBottom:14, color:T.green, fontSize:15, fontFamily:T.font, fontWeight:700 }}>
                    ...
                    <span style={{ color:T.text }}>
                      {subpasso.tipo === "numero" ? fmt(subpasso.esperado, 6) : subpasso.esperado}
                    </span>
                  </div>
                )}
                ...
                {!respondido && (
                  subpasso.tipo === "simnao" ? (
                    <div style={{ display:"flex", gap:10 }}>
                      {["Sim","Não"].map(op => (
                        <button key={op} onClick={() => confirmar(op)} style={{ background: op==="Sim" ? T.greenLight : T.redLight, color: op==="Sim" ? T.green : T.red, border:`1.5px solid ${op==="Sim" ? T.greenBorder : T.redBorder}`, borderRadius:T.radiusSm, padding:"10px 30px", fontSize:15, fontWeight:700, cursor:"pointer", fontFamily:T.fontSans }}>
                          {op}
                        </button>
                      ))}
                    </div>
                    ...
                    <div style={{ display:"flex", gap:10 }}>
                      {subpasso.opcoes.map(op => (
                        <button key={op} onClick={() => confirmar(op)} style={{ background:T.primaryLight, color:T.primary, border:`1.5px solid ${T.primaryBorder}`, borderRadius:T.radiusSm, padding:"10px 36px", fontSize:17, fontWeight:700, cursor:"pointer", fontFamily:T.font }}>
                          {op}
                        </button>
                      ))}
                    </div>
                    ...
                    <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                      <input type="text" value={inputVal} onChange={e => setInputVal(e.target.value)} onKeyDown={e => { if (e.key==="Enter") confirmar(); }} placeholder="Sua resposta numérica" autoFocus
                        style={{ flex:1, maxWidth:260, background:T.bgSection, border:`1.5px solid ${T.borderMed}`, borderRadius:T.radiusSm, color:T.text, fontSize:16, fontFamily:T.font, padding:"10px 14px", transition:"border-color 0.2s, box-shadow 0.2s" }} />
