...
function ResultadoGaussSeidel({ resultado, modo }) {
...
export default function GaussSeidel() {
  ...
  const { calcular, resultado, erro: erroApi, carregando, limpar } = useGaussSeidel();

  function mudarTamanho(n) {
    setTamanho(n); setMatrizA(gerarMatriz(n)); setVetorB(gerarVetor(n)); setVetorX0(gerarVetor(n));
    limpar(); setErroVal("");
  }
  ...
  return (
    <div style={{ padding: 24, fontFamily: "sans-serif" }}>
      <h2 style={{ marginBottom: 8 }}>Método de Gauss-Seidel</h2>
      <p style={{ fontSize: 13, color: "#888", marginBottom: 20, lineHeight: 1.5 }}>
        Variação de Jacobi que usa os valores mais recentes <em>imediatamente</em>: ao calcular
        x<sub>i</sub>, os valores x<sub>1</sub>…x<sub>i-1</sub> já atualizados nesta iteração
        são utilizados, acelerando a convergência.
      </p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {matrizA.map((row, i) => (
              <div key={i} style={{ display: "flex", gap: 4 }}>
                {row.map((v, j) => (
                  <CelulaInput key={j} value={v} onChange={val => setA(i, j, val)} placeholder={`a${i+1}${j+1}`} destaque={i === j} />
                ))}
              </div>
            ))}
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          ...
          {MODOS.map(({ value, label }) => (
            <label key={value} style={{
              ...
              <input type="radio" name="modoGS" value={value} checked={modo === value}
      ...
      {resultado && modo !== "interativo" && (
        <ResultadoGaussSeidel resultado={resultado} modo={modo} />
      )}

      {iteracoes.length > 0 && modo === "interativo" && params && (
        <GaussSeidelInterativo
          key={chaveInt}
